...
```{r}
cluster_url <- "https://raw.githubusercontent.com/rufletch/p63-HBC-diff/master/ref/oeHBCdiff_clusterLabels.txt"
...
We put this all together in a `SingleCellExperiment` object, with some optimizations for space.
...
Maintainer note: cluster labels were obtained from the corresponding code repository on GitHub. Only a subset of cells were used for clustering and thus have these labels; these are indicated as such via the `retained` column.",
        ...
        sources=list(
            ...
            list(provider="other", id=cluster_url)
        ),
        maintainer.name="Davide Risso",
        maintainer.email="risso.davide@gmail.com"
    )
)
```
