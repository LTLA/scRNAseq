...
We polish the datasets to optimaize for space.
...
saveMetadata(cellranger, file.path("finished", "cellranger"), copy)

copy <- meta
copy$title <- sprintf(copy$title, "EmptyDrops")
copy$description <- sprintf(copy$description, "EmptyDrops")
saveMetadata(cellranger, file.path("finished", "emptydrops"), copy)
```
