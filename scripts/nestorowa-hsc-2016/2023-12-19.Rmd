...
```{r}
celltype.file <- bfcrpath(bfc, "http://blood.stemcells.cam.ac.uk/data/all_cell_types.txt")
...
```{r}
flowcyt.file <- bfcrpath(bfc, "http://blood.stemcells.cam.ac.uk/data/coordinates_gene_counts_flow_cytometry.txt.gz")
...
We assemble all of this into a `SingleCellExperiment` object:
...
This dataset is represented as a `SingleCellExperiment` where the FACS data is stored as an alternative experiment and the diffusion map is stored in the reduced dimension. The cell type assignments are stored in the column data, transformed from the identity matrix provided by the authors into a more user-friendly form. The gating strategy for FACS is inferred from the cell names: Prog (Lin- Sca1- c-Kit+), HSPC (Lin- Sca1+ c-Kit+) and LT-HSC (Lin- c-Kit+ Sca1+ CD34- Flk2-).",
        taxonomy.id="10090",
        genome="GRCm38",
        sources=list(
            list(provider="GEO", id="GSE81682"),
            list(provider="PubMed", id="27365425"),
            list(provider="other", id="https://blood.stemcells.cam.ac.uk/data")
        ),
