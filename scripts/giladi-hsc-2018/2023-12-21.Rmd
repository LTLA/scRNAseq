...
meta <- meta_raw
meta$description <- sprintf(meta$description, "RNA", "RNA")
meta$sources <- c(meta$sources, list(list(provider="GEO", id="GSE92575")))
saveDataset(sce, file.path("finished", "rna"), meta)

meta <- meta_raw
meta$description <- sprintf(meta$description, "CRISPR", "CRISPR")
meta$sources <- c(meta$sources, list(list(provider="GEO", id="GSE11349")))
saveDataset(crisp.sce, file.path("finished", "crispr"), meta)
```
