...
FUN <- function(path, as.csv=FALSE, skip=0) {
    ...
    counts <- as(counts[,-(1:2)], "SVT_SparseMatrix")
    type(counts) <- "integer"
...
title <- "[subset] Molecular Diversity of Midbrain Development in Mouse, Human, and Stem Cells"
...
savePart <- function(sce, prefix, details, human=TRUE) {
    saveDataset(sce, file.path("finished", prefix), createMetadata(
        title=title,
        description=paste0(description, "\n\n", details),
        taxonomy.id=if (human) "9606" else "10090",
...
```{r}
savePart(es.data, "human-es", "This dataset contains the human embryonic stem cells from this study.")
savePart(embryo.data, "human-embryo", "This dataset contains the human embryo ventral midbrain cells from this study.")
savePart(ips.data, "human-ips", "This dataset contains the human induced pluripotent stem cells from this study.")
savePart(madult.data, "mouse-adult", "This dataset contains the adult mouse brain cells from this study.")
savePart(membryo.data, "mouse-embryo", "This dataset contains the mouse embryonic brain cells from this study.")
```
