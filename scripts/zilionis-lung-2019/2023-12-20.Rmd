...
We now assemble a `SingleCellExperiment`, moving the SPRING coordinates into the reduced dimensions for easier viewing.

```{r}
sce <- SingleCellExperiment(list(counts=counts))

# Hacking out the spring coordinates.
cn <- colnames(colData)
is.x <- grepl("^x_", cn)
is.y <- grepl("^y_", cn)
which.x <- which(is.x)
...
```{r}
meta <- createMetadata(
    title="[subset] Single-Cell Transcriptomics of Human and Mouse Lung Cancers Reveals Conserved Myeloid Populations across Individuals and Species",
    ...
    maintainer.name="Jens Preussner",
    maintainer.email="jens.preussner@mpi-bn.mpg.de"
)

copy <- meta
copy$taxonomy_id <- "9606"
copy$genome <- "GRCh38"
copy$description <- paste0(copy$description, "\n\n", "This dataset contains the human cells from this study")
...
copy$genome <- "GRCm38"
copy$description <- paste0(copy$description, "\n\n", "This dataset contains the mouse cells from this study.")
