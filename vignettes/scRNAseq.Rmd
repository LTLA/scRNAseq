...
Users can also contribute their own published datasets to this package to enable re-use by the wider Bioconductor community. 

# Finding datasets 

The `surveyDatasets()` function will show all available datasets along with their metadata.
This can be used to discover interesting datasets for `fetchDataset()`.
...
3. Run `saveDataset()` on your `SummarizedExperiment` or whatever object contains your dataset.
...
4. Open a [pull request (PR)](https://github.com/LTLA/scRNAseq/pulls) to request a new dataset.
   ...
   - The code used to assemble the dataset as an Rmarkdown file.
...
5. Run `uploadDirectory()` to upload your staging directory.
   The first time this function is called, it will automatically prompt you to log into GitHub so that the [**gypsum** backend](https://github.com/ArtifactDB/gypsum-worker) can authenticate you.
   If you are on a system without browser access (e.g., most computing clusters), a [token](https://github.com/settings/tokens) can be manually supplied via `gypsum::accessToken()`.

    ```{r, eval=FALSE}
    uploadDirectory(staging, "my_dataset_name", "2020-02-20")
    ```

   You can check that everything was successfully uploaded by just calling `fetchDataset()`:

    ```{r, eval=FALSE}
    fetchDataset("my_dataset_name", "2020-02-20")
    ```
   ...
   Use the following call to clear the erroneous dataset, and try again:
    ...
    ```{r, eval=FALSE}
    gypsum::rejectProbation("scRNAseq", "my_dataset_name", "2020-02-20")
    ```

6. Comment on the PR to notify us that the dataset has finished uploading and you're happy with it.
   We'll review it and make sure everything's in order before approving it.
