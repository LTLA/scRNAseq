...
We obtain a single-cell RNA sequencing dataset of human and mouse lung cancer from .
...
library(Matrix)
FUN <- function(X) {
    input <- read.csv(X, sep="\t", stringsAsFactors=FALSE)
...
```{r}
samples <- sapply(stringi::stri_split_fixed(hs.files, pattern = "_"), function(x) x[3])
barcode <- lapply(all.human, function(x) x$label)
origin <- rep(samples, times = sapply(barcode, length))
donor <- stringi::stri_match_first_regex(origin, "p[0-9]+")[,1]
replicate <- stringi::stri_match_first_regex(origin, "[t|b][0-9]+")[,1]
tissue <- ifelse(grepl("t", replicate), "tumor", "blood")
barcode <- unlist(barcode)

library(S4Vectors)
coldata <- DataFrame(library=origin, donor=donor, replicate = replicate, tissue=tissue, barcode=barcode)
coldata
```
...
```{r}
path <- file.path("scRNAseq", "zilionis-lung", "2.2.0")
...
```{r}
animal <- sapply(stringi::stri_split_fixed(mm.files, pattern = "_"), function(x) paste0(x[3], x[4]))
replicate <- sapply(stringi::stri_split_fixed(mm.files, pattern = "_"), function(x) x[5])
barcode <- lapply(all.mouse, function(x) x$label)

animal <- rep(animal, times = sapply(barcode, length))
replicate <- rep(replicate, times = sapply(barcode, length))
library <- paste(animal, replicate, sep = "_")
tissue <- ifelse(grepl("t", library), "tumor", "healthy")
barcode <- unlist(barcode)

library(S4Vectors)
coldata <- DataFrame(library=library, animal = animal, replicate = replicate, tissue=tissue, barcode=barcode)
coldata
```
