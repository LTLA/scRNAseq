...
read.table(tarball, sep = "\t", header = T) %>% 
  dplyr::left_join(coldata, ., by = c("library" = "Library", "barcode" = "Barcode")) %>% 
  DataFrame(.) -> colData
colData
```
...
```{r}
keep <- grep("counts|cell.type|^x_.*_all|^y_.*_all", colnames(colData))
...
```{r}
keep <- grep("counts|cell.type|^x$|^y$", colnames(colData))
colData <- colData[, c(1:5, keep)]
```
