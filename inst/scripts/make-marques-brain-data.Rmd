...
We obtain a single-cell RNA sequencing dataset of mouse oligodendrocytes from @romanov2017molecular.
...
```{r}
uninteresting <- lapply(coldata, FUN=function(x) length(unique(x)))
keep <- unlist(uninteresting)!=1L
...
colnames(coldata) <- sub(":ch1$", "", colnames(coldata))
coldata
```
