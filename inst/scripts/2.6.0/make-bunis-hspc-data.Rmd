...
authors_meta <- readRDS(full.path)@meta.data

# Add 'retained' metadata representing barcodes retained by authors.
sce$retained <- colnames(sce) %in% rownames(authors_meta)

# Now add NAs to this metadata to account for all the missing "cells"
NAadd <- matrix(NA, ncol = ncol(authors_meta), nrow = sum(!sce$retained))
colnames(NAadd) <- colnames(authors_meta)
rownames(NAadd) <- colnames(sce)[!sce$retained]
authors_meta <- rbind(authors_meta, NAadd)
# Rearrange to be in same order
authors_meta <- authors_meta[colnames(sce),]
