...
# Spike-in concenctrations

We can add the spike-in molecule counts based on the spike-in levels.

```{r}
file <- system.file("inst/extdata/spike-concs.tsv", package = "scRNAseq")
table <- read.delim(file, stringsAsFactors = FALSE)
row <- table[grep("Zeisel", table$Reference), ]
molecule_counts <- scRNAseq:::.calculate_spikeins(
    volume_nl = row$Volume,
    dilution = as.numeric(row$Dilution),
    genes = rownames(spike.data)
)
rowData(spike.data)$molecule_counts <- molecule_counts
```
